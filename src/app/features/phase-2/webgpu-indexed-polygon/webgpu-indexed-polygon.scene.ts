...
export function createWebGpuIndexedPolygonSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-webgpu-indexed-polygon-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-webgpu-indexed-polygon-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
  ...
  const vertexBuffer = runtime.device.createBuffer({
    label: "visual-math-webgpu-indexed-polygon-vertices",
    size: MAX_VERTEX_COUNT * VERTEX_STRIDE,
    usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
  })
  const indexBuffer = runtime.device.createBuffer({
    label: "visual-math-webgpu-indexed-polygon-indices",
