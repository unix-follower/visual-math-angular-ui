...
const WEBGPU_INDIRECT_INDEXED_POLYGON_ROUTE_PATH = "/phase-2/webgpu-indirect-indexed-polygon"
const RANGE_CONTROLS: readonly RangeControlSchema[] = [
  ...
  { kind: "range", id: "coverage", label: "Encoded index coverage", min: 0.34, max: 1, step: 0.01 },
]
...
const KEYBOARD_SHORTCUTS: readonly WorkbenchKeyboardShortcut[] = [
  ...
  { keys: "C / Shift+C", description: "Increase or decrease the encoded index coverage." },
  { keys: "Escape", description: "Reset to the default indirect indexed polygon scene." },
]

@Component({
  selector: "app-webgpu-indirect-indexed-polygon-page",
  imports: [
  ...
  templateUrl: "./webgpu-indirect-indexed-polygon.page.html",
  styleUrl: "./webgpu-indirect-indexed-polygon.page.css",
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WebGpuIndirectIndexedPolygonPageComponent implements OnDestroy {
  private readonly platformId = inject(PLATFORM_ID)
  private readonly route = inject(ActivatedRoute)
  private initializationStarted = false

  protected readonly canvas = viewChild<ElementRef<HTMLCanvasElement>>("canvas")
  protected readonly red = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.red)
  protected readonly green = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.green)
  protected readonly blue = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.blue)
  protected readonly alpha = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.alpha)
  protected readonly sides = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.sides)
  protected readonly radius = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.radius)
  protected readonly rotation = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.rotation)
  protected readonly intensity = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.intensity)
  protected readonly coverage = signal(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE.coverage)
  ...
  protected readonly highlights = [
    "Thirteenth Angular Phase 2 route combining index buffers with drawIndexedIndirect submission",
  ...
  private readonly rangeControlAdapters: Record<string, RangeControlAdapter> = {
    ...
    coverage: {
      value: () => this.coverage(),
      set: (value) => this.coverage.set(clampCoverage(value)),
      displayValue: (value) => value.toFixed(2),
    },
  }
  protected readonly scene = computed<WebGpuIndirectIndexedPolygonScene>(() => ({
    red: this.red(),
    green: this.green(),
    blue: this.blue(),
    alpha: this.alpha(),
  ...
  protected readonly metrics = computed(() => [
    ...
    { label: "Clear color", value: indirectIndexedPolygonClearColor(this.scene()) },
    { label: "Peak color", value: indirectIndexedPolygonPeakColor(this.scene()) },
    { label: "Vertices", value: `${indirectIndexedPolygonVertexCount(this.scene())}` },
    { label: "Full indices", value: `${indirectIndexedPolygonIndexCount(this.scene())}` },
    { label: "Encoded indices", value: `${indirectIndexedPolygonEncodedIndexCount(this.scene())}` },
    { label: "Area", value: indirectIndexedPolygonArea(this.scene()) },
    { label: "Draw path", value: indirectIndexedPolygonStageLabel(this.scene()) },
  ...
  constructor() {
    ...
    if (initialScene) {
      ...
      this.statusMessage.set("Loaded the indirect indexed polygon scene from the shared URL.")
    }
    ...
    effect(() => {
      ...
      renderWebGpuIndirectIndexedPolygonScene(canvasRef.nativeElement, runtime, scene)
      this.statusMessage.set(`Submitted an indirect indexed WebGPU draw using ${runtime.format}.`)
    })
  }
    ...
    switch (actionId) {
      case "copy-share-link": {
        const wasCopied = await copyWorkbenchText(
          buildWorkbenchShareUrl(WEBGPU_INDIRECT_INDEXED_POLYGON_ROUTE_PATH, this.scene()),
        )
      ...
      case "export-png": {
        ...
        const didDownload = canvasRef
          ? downloadWorkbenchCanvas(canvasRef.nativeElement, "webgpu-indirect-indexed-polygon.png")
          : false
        this.statusMessage.set(
      ...
      default:
        this.applyScene(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE)
        this.statusMessage.set("Indirect indexed polygon scene reset to the default preset.")
    }
  }
  ...
  protected handleViewportKeydown(event: KeyboardEvent): void {
    ...
    switch (key) {
      ...
      case "c":
        event.preventDefault()
        stepNumericSignal(this.coverage, event.shiftKey ? -0.05 : 0.05, clampCoverage)
        this.statusMessage.set("Updated the encoded index coverage.")
        break
      case "escape":
        event.preventDefault()
        this.applyScene(DEFAULT_WEBGPU_INDIRECT_INDEXED_POLYGON_SCENE)
  ...
  private applyScene(scene: WebGpuIndirectIndexedPolygonScene): void {
    ...
    this.coverage.set(clampCoverage(scene.coverage))
  }
...
function clampCoverage(value: number): number {
  return Math.min(1, Math.max(0.34, Number(value.toFixed(2))))
}
