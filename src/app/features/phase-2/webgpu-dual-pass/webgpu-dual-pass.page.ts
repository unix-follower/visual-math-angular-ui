...
const WEBGPU_DUAL_PASS_ROUTE_PATH = "/phase-2/webgpu-dual-pass"
...
const ACTIONS: readonly WorkbenchAction[] = [
  {
    ...
    description: "Copy a URL with the current serialized dual-pass WebGPU scene.",
  },
  {
    id: "export-png",
    label: "Export PNG",
...
@Component({
  selector: "app-webgpu-dual-pass-page",
  imports: [
  ...
  templateUrl: "./webgpu-dual-pass.page.html",
  styleUrl: "./webgpu-dual-pass.page.css",
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WebGpuDualPassPageComponent implements OnDestroy {
  private readonly platformId = inject(PLATFORM_ID)
  private readonly route = inject(ActivatedRoute)
  private initializationStarted = false

  protected readonly canvas = viewChild<ElementRef<HTMLCanvasElement>>("canvas")
  protected readonly red = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.red)
  protected readonly green = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.green)
  protected readonly blue = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.blue)
  protected readonly alpha = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.alpha)
  protected readonly glow = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.glow)
  protected readonly skew = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.skew)
  protected readonly mix = signal(DEFAULT_WEBGPU_DUAL_PASS_SCENE.mix)
  ...
  protected readonly statusMessage = signal(
    "Mounting the dual-pass route checks browser support and prepares an offscreen render target.",
  )
  protected readonly highlights = [
    "Ninth Angular Phase 2 route introducing a true multipass render flow instead of a single render pass",
    "First pass draws colored geometry into an offscreen texture and the second pass composites that texture onto the canvas",
  ...
  protected readonly scene = computed<WebGpuDualPassScene>(() => ({
    red: this.red(),
    green: this.green(),
    blue: this.blue(),
    alpha: this.alpha(),
  ...
  protected readonly summary = computed(() =>
    webGpuDualPassSummary(this.scene(), this.runtimeStatusLabel(), this.runtime()?.format),
  )
  protected readonly metrics = computed(() => [
    { label: "Runtime", value: this.runtimeStatusLabel() },
    { label: "Canvas format", value: this.runtime()?.format ?? "Unavailable" },
    { label: "Clear color", value: dualPassClearColor(this.scene()) },
    { label: "Accent color", value: dualPassAccentColor(this.scene()) },
    { label: "Stages", value: dualPassStageLabel(this.scene()) },
  ...
  constructor() {
    ...
    if (initialScene) {
      ...
      this.statusMessage.set("Loaded the dual-pass scene from the shared URL.")
    }
    ...
    effect(() => {
      ...
      renderWebGpuDualPassScene(canvasRef.nativeElement, runtime, scene)
      this.statusMessage.set(`Submitted a multipass WebGPU draw using ${runtime.format}.`)
    })
  }
    ...
    switch (actionId) {
      case "copy-share-link": {
        const wasCopied = await copyWorkbenchText(
          buildWorkbenchShareUrl(WEBGPU_DUAL_PASS_ROUTE_PATH, this.scene()),
        )
      ...
      case "export-png": {
        ...
        const didDownload = canvasRef
          ? downloadWorkbenchCanvas(canvasRef.nativeElement, "webgpu-dual-pass.png")
          : false
        this.statusMessage.set(
      ...
      default:
        this.applyScene(DEFAULT_WEBGPU_DUAL_PASS_SCENE)
        this.statusMessage.set("Dual-pass scene reset to the default preset.")
    }
  }
  ...
  private applyScene(scene: WebGpuDualPassScene): void {
