...
import { WebGpuDualPassPageComponent } from "./webgpu-dual-pass.page"
import {
  releaseWebGpuDualPassResources,
  renderWebGpuDualPassScene,
} from "./webgpu-dual-pass.renderer"

describe("WebGpuDualPassPageComponent", () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [WebGpuDualPassPageComponent],
      providers: [
  ...
  it("releases dual-pass resources and tears down the runtime on destroy", () => {
    const fixture = TestBed.createComponent(WebGpuDualPassPageComponent)
    const component = fixture.componentInstance as unknown as {
      runtime: { set: (value: unknown) => void }
    }
    const geometryDestroy = vi.fn()
    const quadDestroy = vi.fn()
    ...
    const runtime = {
      ...
      device: {
        ...
        createRenderPipeline: vi
          .fn()
          .mockReturnValueOnce({})
          .mockReturnValueOnce({ getBindGroupLayout: vi.fn().mockReturnValue({}) }),
        createBuffer: vi
          .fn()
          .mockReturnValueOnce({ destroy: geometryDestroy })
          .mockReturnValueOnce({ destroy: quadDestroy }),
    ...
    expect(geometryDestroy).toHaveBeenCalledTimes(1)
    expect(quadDestroy).toHaveBeenCalledTimes(1)
