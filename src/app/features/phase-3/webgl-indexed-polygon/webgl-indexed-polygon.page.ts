...
const WEBGL_INDEXED_POLYGON_ROUTE_PATH = "/phase-3/webgl-indexed-polygon"
const RANGE_CONTROLS: readonly RangeControlSchema[] = [
  ...
  { kind: "range", id: "radius", label: "Radius", min: 0.32, max: 0.84, step: 0.01 },
...
@Component({
  selector: "app-webgl-indexed-polygon-page",
  imports: [
  ...
  templateUrl: "./webgl-indexed-polygon.page.html",
  styleUrl: "./webgl-indexed-polygon.page.css",
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WebGlIndexedPolygonPageComponent implements OnDestroy {
  private readonly platformId = inject(PLATFORM_ID)
  private readonly route = inject(ActivatedRoute)
  private initializationStarted = false

  protected readonly canvas = viewChild<ElementRef<HTMLCanvasElement>>("canvas")
  protected readonly red = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.red)
  protected readonly green = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.green)
  protected readonly blue = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.blue)
  protected readonly alpha = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.alpha)
  protected readonly sides = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.sides)
  protected readonly radius = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.radius)
  protected readonly rotation = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.rotation)
  protected readonly intensity = signal(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE.intensity)
  ...
  protected readonly statusMessage = signal(
    "Mounting the WebGL indexed polygon route prepares shared vertices and an index buffer for drawElements.",
  )
  protected readonly highlights = [
    "First Phase 3 route using drawElements instead of pure sequential draws",
    "Shared vertices plus a uint16 index buffer eliminate duplicated perimeter positions",
    "Reuses the extracted WebGL binding helpers for attribute lookup and interleaved pointer setup",
  ]
  ...
  protected readonly scene = computed<WebGlIndexedPolygonScene>(() => ({
    red: this.red(),
    green: this.green(),
    blue: this.blue(),
    alpha: this.alpha(),
  ...
  protected readonly summary = computed(() =>
    webGlIndexedPolygonSummary(this.scene(), this.runtimeStatusLabel(), this.runtime()?.version),
  )
  protected readonly metrics = computed(() => [
    { label: "Runtime", value: this.runtimeStatusLabel() },
    { label: "Context version", value: this.runtime()?.version ?? "Unavailable" },
    { label: "Clear color", value: webGlIndexedPolygonClearColor(this.scene()) },
    { label: "Peak color", value: webGlIndexedPolygonPeakColor(this.scene()) },
    { label: "Vertices", value: `${webGlIndexedPolygonVertexCount(this.scene())}` },
    { label: "Indices", value: `${webGlIndexedPolygonIndexCount(this.scene())}` },
    { label: "Area", value: webGlIndexedPolygonArea(this.scene()) },
  ...
  constructor() {
    ...
    if (initialScene) {
      ...
      this.statusMessage.set("Loaded the WebGL indexed polygon scene from the shared URL.")
    }
    ...
    effect(() => {
      ...
      renderWebGlIndexedPolygonScene(canvasRef.nativeElement, runtime, scene)
      this.statusMessage.set(`Submitted an indexed WebGL polygon draw using ${runtime.version}.`)
    })
  }
    ...
    switch (actionId) {
      case "copy-share-link": {
        const wasCopied = await copyWorkbenchText(
          buildWorkbenchShareUrl(WEBGL_INDEXED_POLYGON_ROUTE_PATH, this.scene()),
        )
      ...
      case "export-png": {
        ...
        const didDownload = canvasRef
          ? downloadWorkbenchCanvas(canvasRef.nativeElement, "webgl-indexed-polygon.png")
          : false
        this.statusMessage.set(
      ...
      default:
        this.applyScene(DEFAULT_WEBGL_INDEXED_POLYGON_SCENE)
        this.statusMessage.set("WebGL indexed polygon scene reset to the default preset.")
    }
  }
  ...
  protected handleViewportKeydown(event: KeyboardEvent): void {
    ...
    switch (key) {
      ...
      case "d":
        event.preventDefault()
        stepNumericSignal(this.radius, event.shiftKey ? -0.04 : 0.04, clampRadius)
  ...
  private applyScene(scene: WebGlIndexedPolygonScene): void {
